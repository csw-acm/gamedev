...
public class MovementController : MonoBehaviour {
	...
	void Update () {
        ...
        {
            cubie.position = orgposition + forward;
        }

        // Move Left
        if (Input.GetKey(KeyCode.A))
        {
            cubie.position = orgposition + left;
        }

        // Move Right
        if (Input.GetKey(KeyCode.D))
        {
            cubie.position = orgposition + right;
        }

        // Move Backward
        if (Input.GetKey(KeyCode.S))
        {
            cubie.position = orgposition + backwards;
        }

        // Make sure we update orgposition so it is the current position of the object
        orgposition = cubie.position;
